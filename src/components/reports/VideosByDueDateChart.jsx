...
import { localDateStr, parseDurationSec, formatDur, durationColor, BRACKETS } from './videoDurationUtils';
...
function VideosByDueDateChart({ tasks, onTaskSelect, onDateClick }) {
  ...
  const videoTasks = useMemo(() => {
    ...
    return tasks.filter(t =>
      t.status !== 'completed' && parseDurationSec(t.notes) > 0 && !!t.due
    );
  }, [tasks]);
  ...
  const chartSec = dataPoints.reduce((s, p) => s + p.videos.reduce((ss, v) => ss + v.durationSec, 0), 0);
  const totalVideos = dataPoints.reduce((s, p) => s + p.videos.length, 0);
  const totalMin = Math.round(chartSec / 60);
  ...
  if (videoTasks.length === 0) {
    return (
      <div className="card">
        <p style={{ color: 'var(--text-secondary)', margin: 0 }}>No video tasks with due dates found.</p>
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      <div className="card" style={{ marginBottom: 'var(--spacing-sm, 8px)' }}>
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <div className="form-group" style={{ marginBottom: 0 }}>
            <label htmlFor="vbdd-start">From</label>
            <input
              id="vbdd-start"
              type="date"
          ...
          <div className="form-group" style={{ marginBottom: 0 }}>
            <label htmlFor="vbdd-end">To</label>
            <input
              id="vbdd-end"
              type="date"
              value={effectiveEnd}
          ...
          <div className="form-group" style={{ marginBottom: 0 }}>
            <label htmlFor="vbdd-range">Range</label>
            <select
              id="vbdd-range"
              value={rangeMode}
          ...
          <div className="form-group" style={{ marginBottom: 0 }}>
            <label htmlFor="vbdd-target">Target</label>
            <select
              id="vbdd-target"
              value={target}
      ...
      <div className="card">
        {totalVideos === 0 && dataPoints.length > 0 ? (
          ...
          <>
            <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary, #4a5568)', marginBottom: '6px' }}>Video tasks by due date</div>
            <svg
                  ...
                  <g key={`bar-${p.date}`}>
                    {stack.map((seg, vi) => (
                      <rect
                        key={`seg-${vi}`}
                        x={x} y={seg.y}
                        width={barWidth} height={seg.barH}
                ...
                return (
                  <>
                    <line
                      x1={padding.left} y1={y}
                      x2={padding.left + chartW} y2={y}
                      stroke="#e53e3e" strokeWidth="1.5" strokeDasharray="6,4"
                    />
                    <text
                      x={padding.left + chartW + 4} y={y - 3}
                      textAnchor="start" fontSize="10" fill="#e53e3e"
                    >
                      {label}
                    </text>
                  </>
...
export default VideosByDueDateChart;
