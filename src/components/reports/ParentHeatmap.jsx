...
function ParentHeatmap({ tasks, onDateClick, dateStart, dateEnd, selectedParentId }) {
  ...
  // Dates where the selected parent's children have due dates
  const parentChildDates = useMemo(() => {
    if (!selectedParentId) return new Set();
    const s = new Set();
    tasks.forEach(t => {
  ...
  // Due date of the selected parent itself
                ...
                {cells.map(cell => {
                  ...
                  return (
                    <g key={cell.date} style={{ cursor: 'pointer' }} onClick={() => onDateClick && onDateClick(cell.date)}>
                      ...
                      {/* Subtask dot (when no parent selected) */}
